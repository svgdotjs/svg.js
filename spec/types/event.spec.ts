...
describe('Event', function () {
  ...
  describe('on()', function () {
    ...
    it('attaches an event to a non svg element', function () {
      ...
      SVGJS.on(el, 'event', action)
      el.dispatchEvent(new CustomEvent('event'))
      expect(toast).toBe('ready')
      SVGJS.off(el, 'event', action)
    })
    it('attaches multiple handlers on different element', function () {
      var rect2 = helpers.draw.rect(100, 100)
      var rect3 = helpers.draw.rect(100, 100)
      var butter: string
    ...
    it('attaches a handler to a namespaced event', function () {
      ...
      expect(rect.events['event']['*']).toBeUndefined()          // no global listener on rect
      expect(Object.keys(rect.events['event']['namespace1']).length).toBe(1) // 1 namespaced listener on rect
      expect(Object.keys(rect2.events['event']['namespace2']).length).toBe(1) // 1 namespaced listener on rect2
    ...
    it('stores the listener for future reference', function () {
      rect.on('event', action)
      expect(rect.events['event']['*'][(action as any)._svgjsListenerId]).not.toBeUndefined()
    })
  ...
  describe('off()', function () {
    ...
    it('detaches a specific event listener, all other still working', function () {
      ...
      const rect3 = helpers.draw.rect(100, 100)
