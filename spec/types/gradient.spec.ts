...
describe('Gradient', function () {
  var rect: SVGJS.Rect, radial: SVGJS.Gradient, gradient: SVGJS.Gradient

  beforeEach(function() {
    rect = helpers.draw.rect(100,100)
    gradient = helpers.draw.gradient('linear', function(add: SVGJS.Gradient) {
      add.stop({ offset: 0, color: '#333', opacity: 1 })
      add.stop({ offset: 1, color: '#fff', opacity: 1 })
    })
    radial = helpers.draw.gradient('radial', function(add) {
      add.stop({ offset: 0, color: '#333', opacity: 1 })
      add.stop({ offset: 1, color: '#fff', opacity: 1 })
    })
  })
  ...
  it('is an instance of SVG.Gradient', function() {
    expect(gradient instanceof SVGJS.Gradient).toBe(true)
  })

  it('allows creation of a new gradient without block', function() {
    gradient = helpers.draw.gradient('linear')
  ...
  describe('input values', function() {
    var s1: SVGJS.Stop, s2: SVGJS.Stop

    it('accepts floats', function() {
      gradient = helpers.draw.gradient('linear', function(add) {
  ...
  describe('update()', function() {
    ...
    it('removes all existing children first', function() {
      gradient = helpers.draw.gradient('linear', function(add) {
        const s1 = add.stop({ offset: 0.12, color: '#333', opacity: 1 })
        const s2 = add.stop({ offset: 0.93, color: '#fff', opacity: 1 })
      })
      expect(gradient.children().length).toBe(2)
      gradient.update(function(add) {
        const s1 = add.stop({ offset: 0.33, color: '#666', opacity: 1 })
        const s2 = add.stop({ offset: 1, color: '#000', opacity: 1 })
      })
      expect(gradient.children().length).toBe(2)
    })
