...
export var drawing: HTMLElement = document.createElement('div')
export var imageUrl: string = 'fixtures/pixel.png'
export var draw: SVGJS.Svg
...
export var updateDraw = function (udraw: SVGJS.Svg) {
   draw = udraw
}

// strip spaces from result
export var stripped = function (str: string) {
   str = str.replace(/\s+/g, '')
...
export var matrixStringToArray = function (source: string) {
   return source
      .replace(/matrix\(|\)/, '')
      .split(SVGJS.regex.delimiter)
      .map(parseFloat)
}

// This is needed because of IE11 creating values like 2.99999 when calculating a transformed box
export var roundBox = function (box: SVGJS.BoxLike) {
   return new SVGJS.Box(
      Math.round(box.x),
      Math.round(box.y),
...
export var roundMatrix = function (mat: SVGJS.MatrixLike) {
   return new SVGJS.Matrix(
      +(mat.a.toFixed(5)),
      +(mat.b.toFixed(5)),
      +(mat.c.toFixed(5)),
      +(mat.d.toFixed(5)),
