...
describe('ClipPath', function () {
  ...
  beforeEach(function () {
    ...
    circle = helpers.draw.circle(100).move(50, 50)
  ...
  it('moves the clipping element to a new clip node', function () {
    expect(circle.parent() instanceof SVGJS.ClipPath).toBe(true)
  })
  ...
  it('references the clip element in the masked element', function () {
    expect(rect.clipper()).toBe(circle.parent() as SVGJS.ClipPath)
  })
  ...
  it('reuses clip element when clip was given', function () {
    ...
    const a = helpers.draw.rect(100, 100).clipWith(clip)
    const b = a.clipper()
    expect(b).toBe(clip)
  })
