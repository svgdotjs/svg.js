...
describe('Pattern', function () {
  var rect: SVGJS.Rect, pattern: SVGJS.Pattern

  beforeEach(function () {
    rect = helpers.draw.rect(100, 100)
    pattern = helpers.draw.pattern(20, 30, function (add) {
  ...
  it('is an instance of SVG.Pattern', function () {
    expect(pattern instanceof SVGJS.Pattern).toBe(true)
  })

  it('allows creation of a new gradient without block', function () {
    pattern = helpers.draw.pattern(10, 30)
  ...
  describe('update()', function () {
    ...
    it('removes all existing children first', function () {
      pattern = helpers.draw.pattern(30, 30, function (add) {
        add.rect(10, 10).move(10, 10)
        add.circle(30)
      })
