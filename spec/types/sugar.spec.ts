...
describe('Sugar', function () {
  ...
  describe('rotate()', function () {
    var rect: SVGJS.Rect, spy: jasmine.Spy

    beforeEach(function () {
      rect = helpers.draw.rect(100, 100)
      spy = spyOn(rect, 'transform')
    })
  ...
  describe('matrix()', function () {
    ...
    it('redirects to attr() directly with one argument', function () {
      ...
      expect(rect.attr).toHaveBeenCalledWith('transform', new SVGJS.Matrix([1, 2, 3, 4, 5, 6]))
    })
  ...
  describe('opacity()', function () {
    var rect: SVGJS.Rect, spy: jasmine.Spy, undefined
  ...
  describe('dx() / dy()', function () {
    var rect: SVGJS.Rect, spyX: jasmine.Spy, spyY: jasmine.Spy

    beforeEach(function () {
      rect = helpers.draw.rect(100, 100)
      spyX = spyOn(rect, 'x').and.callThrough()
      spyY = spyOn(rect, 'y').and.callThrough()
    })
    ...
    it('redirects to x() / y() with adding the current value', function () {
      rect.dx(5)
      rect.dy(5)
      expect(rect.x).toHaveBeenCalledWith(jasmine.objectContaining(new SVGJS.Number('5')))
      expect(rect.y).toHaveBeenCalledWith(jasmine.objectContaining(new SVGJS.Number('5')))
    })
    ...
    it('allows to add a percentage value', function () {
      ...
      expect(rect.x).toHaveBeenCalledWith(jasmine.objectContaining(new SVGJS.Number('10%')))
      expect(rect.y).toHaveBeenCalledWith(jasmine.objectContaining(new SVGJS.Number('10%')))
    })
  ...
  describe('dmove()', function () {
    var rect: SVGJS.Rect, spyDx: jasmine.Spy, spyDy: jasmine.Spy

    beforeEach(function () {
      rect = helpers.draw.rect(100, 100)
      spyDx = spyOn(rect, 'dx').and.callThrough()
      spyDy = spyOn(rect, 'dy').and.callThrough()
    })
  ...
  describe('font()', function () {
    var text: SVGJS.Text, spyLeading: jasmine.Spy, spyAttr: jasmine.Spy

    beforeEach(function () {
      text = helpers.draw.text(helpers.loremIpsum)
      spyLeading = spyOn(text, 'leading')
      spyAttr = spyOn(text, 'attr')
    })

    afterEach(function () {
      text.remove()
      spyLeading.calls.reset()
      spyAttr.calls.reset()
    })
