...
describe('Text', function () {
  ...
  beforeEach(function () {
    text = helpers.draw.text(helpers.loremIpsum).size(5)
  })
  ...
  describe('leading()', function () {
    it('returns the leading value of the text without an argument', function () {
      expect(text.leading() instanceof SVGJS.Number)
  ...
  describe('x()', function () {
    ...
    it('sets the value of all lines', function () {
      text.x(200)
      text.each(function (index: number, cd: SVGJS.Element[]) {
        expect(cd[index].x()).toBe(text.attr('x'))
      })
    })
  })
  ...
  describe('text()', function () {
    it('adds content in a nested tspan', function () {
      ...
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('It is a bear!')
    })
    it('adds content in a nested tspan even with an empty string', function () {
      text.text('')
      expect(text.node.childNodes[0].nodeType).toBe(1)
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('')
    })
    ...
    it('accepts a block as first arguments', function () {
      ...
      expect(text.node.childNodes[0].nodeType).toBe(1)
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('mastaba')
      expect(text.node.childNodes[1].nodeType).toBe(3)
      expect((text.node.childNodes[1] as CharacterData).data).toBe('hut')
    })
  })
  ...
  describe('plain()', function () {
    it('adds content without a tspan', function () {
      ...
      expect((text.node.childNodes[0] as CharacterData).data).toBe('It is a bear!')
    })
    it('clears content before adding new content', function () {
      text.plain('It is not a bear!')
      expect(text.node.childNodes.length).toBe(1)
      expect((text.node.childNodes[0] as CharacterData).data).toBe('It is not a bear!')
    })
    it('restores the content from the dom', function () {
  ...
  describe('tspan()', function () {
    ...
    it('clears content before adding new content', function () {
      ...
      expect(text.node.childNodes.length).toBe(1)
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('It is not a bear!')
    })
  })
  ...
  describe('build()', function () {
    it('enables adding multiple plain text nodes when given true', function () {
      ...
      expect((text.node.childNodes[0] as CharacterData).data).toBe('A great piece!')
      expect((text.node.childNodes[1] as CharacterData).data).toBe('Another great piece!')
    })
    it('enables adding multiple tspan nodes when given true', function () {
      text.clear().build(true)
      text.tspan('A great piece!')
      text.tspan('Another great piece!')
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('A great piece!')
      expect((text.node.childNodes[1].childNodes[0] as CharacterData).data).toBe('Another great piece!')
    })
    it('disables adding multiple plain text nodes when given false', function () {
      text.clear().build(true)
      text.plain('A great piece!')
      text.build(false).plain('Another great piece!')
      expect((text.node.childNodes[0] as CharacterData).data).toBe('Another great piece!')
    ...
    it('disables adding multiple tspan nodes when given false', function () {
      ...
      expect((text.node.childNodes[0].childNodes[0] as CharacterData).data).toBe('Another great piece!')
      expect(text.node.childNodes[1]).toBe(undefined)
    })
  })
  ...
  describe('setData()', function () {
    it('read all data from the svgjs:data attribute and assign it to el.dom', function () {
      ...
      expect(text.dom.leading instanceof SVGJS.Number).toBeTruthy()
