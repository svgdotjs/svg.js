...
describe('Element', function () {
  ...
  describe('attr()', function () {
    ...
    it('creates an image in defs when image path is specified for fill', function () {
      rect.attr('fill', helpers.imageUrl)
      expect(helpers.draw.defs().find('pattern').length).toBe(1)
      expect(helpers.draw.defs().find('pattern image').length).toBe(1)
      expect(helpers.draw.defs().find('pattern image')[0].attr('href')).toBe(helpers.imageUrl)
    })
    it('creates pattern in defs when image object is specified for fill', function () {
      rect.attr('fill', new SVGJS.Image().load(helpers.imageUrl))
      expect(helpers.draw.defs().find('pattern').length).toBe(1)
      expect(helpers.draw.defs().find('pattern image').length).toBe(1)
      expect(helpers.draw.defs().find('pattern image')[0].attr('href')).toBe(helpers.imageUrl)
    })
    ...
    it('redirects to the leading() method when setting leading', function () {
      var text = helpers.draw.text(helpers.loremIpsum)
      spyOn(text, 'leading')
  ...
  describe('css()', function () {
    it('sets the style with key and value arguments', function () {
      var rect = helpers.draw.rect(100, 100).css('cursor', 'crosshair')
  ...
  describe('transform()', function () {
    var rect: SVGJS.Rect, ctm: SVGJS.Matrix
    ...
    it('sets the translation of and element', function () {
      ...
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1, 0, 0, 1, 10, 11])
    })
    it('performs an absolute translation', function () {
      rect.transform({ translate: [10, 11] }).transform({ translate: [20, 21] })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1, 0, 0, 1, 20, 21])
    })
    it('performs a relative translation with relative flag', function () {
      ...
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1, 0, 0, 1, 30, 32])
    })
    it('sets the scaleX and scaleY of an element', function () {
      rect.transform({ scaleX: 0.5, scaleY: 2 })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([0.5, 0, 0, 2, 25, -50])
    })
    it('performs a uniform scale with scale given', function () {
      rect.transform({ scale: 3 })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([3, 0, 0, 3, -100, -100])
    })
    it('also works with only skaleX', function () {
      rect.transform({ scaleX: 3 })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([3, 0, 0, 1, -100, 0])
    })
    it('also works with only skaleY', function () {
      rect.transform({ scaleY: 3 })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1, 0, 0, 3, 0, -100])
    })

    it('performs an absolute scale by default', function () {
      rect.transform({ scale: 3 }).transform({ scale: 0.5 })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([0.5, 0, 0, 0.5, 25, 25])
    })
    it('performs a relative scale with a relative flag', function () {
      ...
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1.5, 0, 0, 8, -25, -350])
    })
    ...
    it('transforms element using a matrix', function () {
      ...
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([0.5, 0, 0.5, 1, 0, 0])
    })
    it('transforms relative using a matrix', function () {
      rect.transform({ a: 0.5, c: 0.5 }).transform(new SVGJS.Matrix({ e: 20, f: 20 } as SVGJS.MatrixLike), true)
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([0.5, 0, 0.5, 1, 20, 20])
    })
    it('flips the element on x axis', function () {
      rect.transform({ flip: 'x' })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([-1, 0, 0, 1, 100, 0])
    })
    it('flips the element on x axis with offset', function () {
      rect.transform({ flip: 'x', origin: [20, 0] })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([-1, 0, 0, 1, 40, 0])
    })
    it('flips the element on y axis with offset', function () {
      rect.transform({ flip: 'y', origin: [0, 20] })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([1, 0, 0, -1, 0, 40])
    })
    it('flips the element on both axis with offset', function () {
      rect.transform({ flip: 'both', origin: [20, 20] })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([-1, 0, 0, -1, 40, 40])
    })
    it('flips the element on both axis', function () {
      rect.transform({ flip: 'both' })
      expect(helpers.matrixStringToArray(rect.node.getAttribute('transform'))).toEqual([-1, 0, 0, -1, 100, 100])
    })
  })

  describe('untransform()', function () {
    var circle: SVGJS.Circle

    beforeEach(function () {
      circle = helpers.draw.circle(100).translate(50, 100)
    })

    it('removes the transform attribute', function () {
      expect(helpers.matrixStringToArray(circle.node.getAttribute('transform'))).toEqual([1, 0, 0, 1, 50, 100])
      circle.untransform()
      expect(circle.node.getAttribute('transform')).toBeNull()
    })
    it('resets the current transform matix', function () {
      expect(circle.ctm()).toEqual(new SVGJS.Matrix(1, 0, 0, 1, 50, 100))
      circle.untransform()
      expect(circle.ctm()).toEqual(new SVGJS.Matrix)
    })
  })
  ...
  describe('toParent()', function () {
    var nested: SVGJS.Svg, g1: SVGJS.G, g2: SVGJS.G, rect1: SVGJS.Rect, rect2: SVGJS.Rect

    beforeEach(function () {
      nested = helpers.draw.nested()
    ...
    it('moves the element to other parent while maintaining the same visal representation', function () {
      expect(helpers.roundMatrix(rect1.toParent(nested).matrix()))
        .toEqual(new SVGJS.Matrix(2, 0, 0, 2, 70, 70))
      expect(rect1.parent()).toEqual(nested)
      expect(helpers.roundMatrix(rect2.toParent(g2).matrix()))
        .toEqual(new SVGJS.Matrix(0.5, 0, 0, 0.5, -95, -95))
      expect(rect2.parent()).toEqual(g2)
    })
  })

  describe('toRoot()', function () {
    var nested: SVGJS.Svg, g1: SVGJS.G, g2: SVGJS.G, rect: SVGJS.Rect
  ...
  describe('ctm()', function () {
    ...
    it('returns an instance of SVG.Matrix', function () {
      expect(rect.ctm() instanceof SVGJS.Matrix).toBeTruthy()
    })
  })

  describe('data()', function () {
    it('sets a data attribute and convert value to json', function () {
      var rect = helpers.draw.rect(100, 100).data('test', 'value')
    ...
    it('sets a data attribute and not convert value to json if flagged raw', function () {
      var rect = helpers.draw.rect(100, 100).data('test', 'value', true)
      expect(rect.node.getAttribute('data-test')).toBe('value')
    })
    it('sets multiple data attributes and convert values to json when an object is passed', function () {
      var rect = helpers.draw.rect(100, 100).data({
        forbidden: 'fruit'
        , multiple: {
          values: 'in'
          , an: 'object'
    ...
    it('gets data value if only one argument is passed', function () {
      var rect = helpers.draw.rect(100, 100).data('test', 101)
      expect(rect.data('test')).toBe(101)
    })
    it('gets the raw value when value is no valid json', function () {
      var rect = helpers.draw.rect(100, 100).data('test', '{["sd":12}]', true)
    ...
    it('maintains data type for an object', function () {
      var rect = helpers.draw.rect(100, 100).data('test', { string: 'value', array: [1, 2, 3] })
      expect(typeof rect.data('test')).toBe('object')
      expect(Array.isArray((rect.data('test') as any).array)).toBe(true)
    })
  })
  ...
  describe('remove()', function () {
    ...
    it('removes an element from its parent', function () {
      var rect = helpers.draw.rect(100, 100)
      rect.remove()
      expect(helpers.draw.has(rect)).toBe(false)
    })
  })
  ...
  describe('rbox()', function () {
    it('returns an instance of SVG.Box', function () {
      var rect = helpers.draw.rect(100, 100)
      expect(rect.rbox() instanceof SVGJS.Box).toBe(true)
    })
    it('returns the correct rectangular box', function () {
      ...
      var rect = helpers.draw.size(200, 150).viewbox(0, 0, 200, 150).rect(105, 210).move(2, 12)//.stroke({width:0})
      var box = rect.rbox(helpers.draw)
  ...
  describe('root()', function () {
    it('returns the parent document', function () {
      var rect = helpers.draw.rect(100, 100)
      expect(rect.root()).toBe(helpers.draw)
    })
  })
  ...
  describe('parent()', function () {
    it('contains the parent svg', function () {
      var rect = helpers.draw.rect(100, 100)
      expect(rect.parent()).toBe(helpers.draw)
    })
  ...
  describe('clone()', function () {
    var rect: SVGJS.Rect, group: SVGJS.G, circle: SVGJS.Circle

    beforeEach(function () {
      rect = helpers.draw.rect(100, 100).center(321, 567).fill('#f06')
      group = helpers.draw.group().add(rect)
  ...
  describe('toString()', function () {
    it('returns the element id', function () {
      var rect = helpers.draw.rect(100, 100).center(321, 567).fill('#f06')
  ...
  describe('replace()', function () {
    it('replaces the original element by another given element', function () {
      var rect = helpers.draw.rect(100, 100).center(321, 567).fill('#f06')
      var circle = helpers.draw.circle(200)
      var rectIndex = helpers.draw.children().indexOf(rect)

      rect.replace(circle)

      expect(rectIndex).toBe(helpers.draw.children().indexOf(circle))
    })
    it('removes the original element', function () {
      var rect = helpers.draw.rect(100, 100).center(321, 567).fill('#f06')

      rect.replace(helpers.draw.circle(200))

      expect(helpers.draw.has(rect)).toBe(false)
    })
    it('returns the new element', function () {
      var circle = helpers.draw.circle(200)
      var element = helpers.draw.rect(100, 100).center(321, 567).fill('#f06').replace(circle)
  ...
  describe('classes()', function () {
    it('returns an array of classes on the node', function () {
      var element = helpers.draw.rect(100, 100)
  ...
  describe('reference()', function () {
    it('gets a referenced element from a given attribute', function () {
      var rect = helpers.draw.defs().rect(100, 100)
        , use = helpers.draw.use(rect)
        , mark = helpers.draw.marker(10, 10)
        , path = helpers.draw.path(helpers.svgPath).marker('end', mark)
  ...
  describe('svg()', function () {
    describe('without an argument', function () {
      it('returns full raw svg when called on the root svg', function () {
        helpers.draw.size(100, 100).rect(100, 100).id(null)
        helpers.draw.circle(100).fill('#f06').id(null)

        var toBeTested = helpers.draw.svg()
      ...
      it('returns partial raw svg when called on a sub group', function () {
        var group = helpers.draw.group().id(null)
    ...
    describe('with raw svg given', function () {
      it('imports a full svg document', function () {
        helpers.draw.svg('<svg id="SvgjsSvg1000" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="100" height="100" viewBox="0 0 50 50"><rect id="SvgjsRect1183" width="100" height="100"></rect><circle id="SvgjsCircle1184" r="50" cx="25" cy="25" fill="#ff0066"></circle></svg>')

        expect(helpers.draw.get(0).type).toBe('svg')
        expect(helpers.draw.get(0).children().length).toBe(2)
        expect(helpers.draw.get(0).get(0).type).toBe('rect')
        expect(helpers.draw.get(0).get(1).type).toBe('circle')
        expect(helpers.draw.get(0).get(1).attr('fill')).toBe('#ff0066')
      })
      it('imports partial svg content', function () {
        helpers.draw.svg('<g id="SvgjsG1185"><rect id="SvgjsRect1186" width="100" height="100"></rect><circle id="SvgjsCircle1187" r="50" cx="25" cy="25" fill="#ff0066"></circle></g>')
        expect(helpers.draw.get(0).type).toBe('g')
        expect(helpers.draw.get(0).get(0).type).toBe('rect')
        expect(helpers.draw.get(0).get(1).type).toBe('circle')
        expect(helpers.draw.get(0).get(1).attr('fill')).toBe('#ff0066')
      })
    ...
    describe('with a modifier function', function () {
      var rect: SVGJS.Rect, circle: SVGJS.Circle, group: SVGJS.G
      beforeEach(function () {
        rect = helpers.draw.rect(10, 10)
        circle = helpers.draw.circle(20, 20)
        group = helpers.draw.group()
      ...
      it('executes the modifier', function () {
        var result = group.svg(function (instance: SVGJS.Element) {
      ...
      it("execute the modifier to replace the node", function () {
        var result = group.svg(function (instance: SVGJS.Element) {
          if (instance instanceof SVGJS.Circle) {
            return helpers.draw.rect(15, 15)
          }
        })
  ...
  describe('writeDataToDom()', function () {
    it('set all properties in el.dom to the svgjs:data attribute', function () {
      var rect = helpers.draw.rect(100, 100)
      rect.dom.foo = 'bar'
      rect.dom.number = new SVGJS.Number('3px')
  ...
  describe('point()', function () {
    it('creates a point from screen coordinates transformed in the elements space', function () {
      ...
      var m = (helpers.draw.node as SVGGraphicsElement).getScreenCTM()
  ...
  describe('show()', function () {
    it('sets display property to ""', function () {
      var rect = helpers.draw.rect(100, 100).show()
      expect(rect.css('display')).toBe('')
    })
  })
  describe('hide()', function () {
    it('sets display property to none', function () {
      var rect = helpers.draw.rect(100, 100).hide()
  ...
  describe('defs()', function () {
    it('returns the defs from the svg', function () {
      var g = helpers.draw.group()
      expect(g.defs()).toBe(helpers.draw.root().defs())
      expect(g.defs() instanceof SVGJS.Defs).toBeTruthy()
    })
  })
  describe('round()', function () {
    it('round all attribues of a node to a specific precision', function () {
      var rect = helpers.draw.rect(100.123456, 200.987654)
  ...
  describe('words()', function () {
    it('inserts plain text in a node', function () {
      var element = helpers.draw.element('title').words('These are some words.').id(null)
  ...
  describe('element()', function () {
    ...
    beforeEach(function () {
      element = helpers.draw.element('rect')
    })

    it('creates an instance of Dom', function () {
      expect(element instanceof SVGJS.Dom).toBeTruthy()
    })
    it('creates element in called parent', function () {
      expect(element.parent()).toBe(helpers.draw)
    })
  })
})
