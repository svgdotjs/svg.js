...
describe('Box', function () {
  describe('initialization', function () {
    ...
    it('creates a new box from parsed string', function () {
      box = new SVGJS.Box('10. 100 200 300')
    ...
    it('creates a new box from object', function () {
      box = new SVGJS.Box({ x: 10, y: 100, width: 200, height: 300 })
    ...
    it('creates a new box from object width left and top instead of x and y', function () {
      box = new SVGJS.Box({ left: 10, top: 100, width: 200, height: 300 })
    ...
    it('creates a new box from parsed string with exponential values', function () {
      box = new SVGJS.Box('-1.12e1 1e-2 +2e2 +.3e+4')
  ...
  describe('merge()', function () {
    it('merges various bounding boxes', function () {
      ...
      var box2 = new SVGJS.Box(300, 400, 100, 100)
      var box3 = new SVGJS.Box(500, 100, 100, 100)
    ...
    it('returns a new instance', function () {
      ...
      expect(merged instanceof SVGJS.Box).toBe(true)
    })
  })

  describe('transform()', function () {
    it('transforms the box with given matrix', function () {
      var box1 = new SVGJS.Box(50, 50, 100, 100).transform(new SVGJS.Matrix(1, 0, 0, 1, 20, 20))
      var box2 = new SVGJS.Box(50, 50, 100, 100).transform(new SVGJS.Matrix(2, 0, 0, 2, 0, 0))
      var box3 = new SVGJS.Box(-200, -200, 100, 100).transform(new SVGJS.Matrix(1, 0, 0, 1, -20, -20))
...
describe('Boxes', function () {
  var rect: SVGJS.Rect, nested: SVGJS.Svg, offset: SVGJS.Matrix

  beforeEach(function () {
    offset = helpers.draw.screenCTM()
    helpers.draw.viewbox(100, 100, 200, 200)
    nested = helpers.draw.nested().size(200, 200).move(100, 100).viewbox(0, 0, 100, 100)
    rect = nested.rect(50, 180).stroke({ width: 0 }).move(25, 90).transform({ scale: 2, origin: [0, 0] }).relative(10, 10)
  })
  afterEach(function () {
    helpers.draw.clear().attr('viewBox', null)
  })

  describe('bbox()', function () {
    it('returns an instance of SVG.Box', function () {
      expect(rect.bbox() instanceof SVGJS.Box).toBeTruthy()
    })
    ...
    it('returns a box even if the element is not in the dom and invisible', function () {
      var line = new SVGJS.Line().plot(0, 0, 50, 50).hide()
      var box = line.bbox()
  ...
  describe('rbox()', function () {
    it('returns an instance of SVG.Box', function () {
      expect(rect.rbox() instanceof SVGJS.Box).toBeTruthy()
    })

    it('returns the elements box in absolute screen coordinates by default', function () {
      var box = rect.rbox()

      expect(helpers.roundBox(box)).toEqual(jasmine.objectContaining(helpers.roundBox({
  ...
  describe('viewbox()', function () {
    ...
    it('should set the viewbox when four arguments are provided', function () {
      helpers.draw.viewbox(0, 0, 100, 100)
      expect(helpers.draw.node.getAttribute('viewBox')).toBe('0 0 100 100')
    })
    it('should set the viewbox when an object is provided as first argument', function () {
      helpers.draw.viewbox({ x: 0, y: 0, width: 50, height: 50 })
      expect(helpers.draw.node.getAttribute('viewBox')).toBe('0 0 50 50')
    })
    it('should set the viewbox when a string is provided as first argument', function () {
      helpers.draw.viewbox('0 0 50 50')
      expect(helpers.draw.node.getAttribute('viewBox')).toBe('0 0 50 50')
    })
    ...
    it('should accept negative values', function () {
      helpers.draw.size(100, 100).viewbox(-100, -100, 50, 50)
      expect(helpers.draw.node.getAttribute('viewBox')).toEqual('-100 -100 50 50')
    })
    it('should get the viewbox if no arguments are given', function () {
      helpers.draw.viewbox(0, 0, 100, 100)
      expect(helpers.draw.viewbox()).toEqual(new SVGJS.Box(0, 0, 100, 100))
    })
    it('should get a nulled viewbox when no viewbox attribute is set', function () {
      expect(helpers.draw.viewbox()).toEqual(new SVGJS.Box())
    })
  })
